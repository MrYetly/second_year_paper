...
\noindent I will define \( \Omega_{12} = \left( \beta_1 / \beta_2 \right)^{\frac{1}{\alpha - 1}}\). Substituting this in and rearranging, we get
...
\begin{equation}
p_j =  \frac{c}{\sum_{i=1}^n \left( \frac{\beta_i}{\beta_j}\right)^{\frac{1}{1- \alpha}}} + v_j -  \frac{\sum_{i =1}^n v_i}{\sum_{i=1}^n \left( \frac{\beta_i}{\beta_j}\right)^{\frac{1}{1 - \alpha}}} 
\label{closed_p}
\end{equation}

In this solution, the optimal distribution of consumer surplus is sensitive to the parameters \(\alpha\), \(\beta_1\), and \(\beta_2\). For very large negative values of \(\alpha\) the weights become meaningless and society values the surplus of each individual equally. In the limit, consumer surpluses are perfect complements and we are left with the following tax schedules.

\[
p_j = \frac{c}{n} +v_j - \frac{1}{n}\sum_{i =1}^n v_i
\]

\noindent When society distributes consumer surplus equally it levies tax schedules with two components: individuals share the marginal cost of providing the public good at every margin of provision, and they are additionally taxed or subsidized according how much more or less they value each margin of provision relative to society' average valuation of that margin. 

As \(\alpha\) approaches 0 from below, the weights begin to have an effect. In the summations of \ref{closed_p}, wherever \(\beta_j>\beta_i\) the summand becomes very large, and wherever \(\beta_j>\beta_i\)

%why is most heavily weighted surplus getting no surplus? CES only defined for alpha<1, if alpha>1 you have a giffen good

two effects manifest themselves. First,  \( \sum_{i=1}^n \left( \frac{\beta_j}{\beta_i}\right)^{\frac{1}{\alpha - 1}}\) becomes larger. If 


Figure \ref{CES_plots} plots some examples of \ref{closed_p} in the same setting used to expose the unjustness of the classical Lindahl equilibrium in section \ref{CLE}: there are two consumers with \(v_1(x) = 20 - 2x\) and \(v_2(x) = 6 - 0.1x\), the marginal cost of providing the public good is 5, and a Pareto optimal amount of it is 10. \(\beta_1\) is 10 and \(\beta_2\) is 1. 

% redo graphs for no alpha>1
...
Inside the brackets, add \(0 = v_j \left( \frac{\beta_j}{\beta_j}\right)^{\frac{1}{\alpha - 1}} - v_j\) and rearrange to get:
