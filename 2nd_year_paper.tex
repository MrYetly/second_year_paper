...
\title{Generalized Lindahl Equilibria and a Generalized Walker Mechanism for Attaining Them}
...
\begin{abstract}
We extend both the concept of Lindahl equilibria and Walker's mechanism for attaining them. Lindahl's original result optimally allocates resources in an economy with a public good, but it does so without taking into account the distribution of consumer surplus. We show this can lead to unjust outcomes. Our extension, generalized Lindahl equilibrium, corrects this issue without losing the Pareto optimality, individual rationality, and balanced budget of the original concept, by giving each consumer a personalized tax schedule. We show how a generalized Lindahl equilibrium can be used to distribute surplus according to preferences embodied in a social welfare function. We also introduce a mechanism capable of attaining generalized Lindahl equilibria when the private valuations of consumers are not known, a generalized Walker mechanism. We prove that any Nash equilibrium of a game induced by a generalized Walker mechanism is a generalized Lindahl equilibrium.
\end{abstract}

	
	The theory of public goods is concerned about what is just. Erik Lindahl's classic result (1919 [1958]) is an argument for how economic resources should be justly allocated in the presence of a public good. He argues that such an allocation should be Pareto optimal, and that individuals should be taxed according to the benefit they receive from the public good such that the sum of all taxes exactly covers the cost of providing it. His equilibrium is just because it is individually rational for individuals to accept the taxes administered to them and because it leaves those individuals as well off as they can be. It has become a standard by which theories of the provision of public goods are measured.
	...
	This paper contributes to a body of work concerned with the extension of Lindahl's original theory. Foley (1970) rigorously stated Lindahl's result and then studied how it related to core theory and showed that all Lindahl equilibria are in the core of an economy.\footnote{The core of an economy is the set of outcomes that optimizing agents converge to in a game theoretic setting in which said agents have the ability to form, and strategize as, coalitions.} Kaneko (1977) pointed out that Lindahl's equilibrium does not dictate how profits should be redistributed, and so in the presence of profits (i.e. non-linear costs) not all Lindahl equilibria are in the core of the economy. He introduced the concept of a ratio equilibrium, in which the total costs of production of public goods are distributed among consumers according to a vector of ratios. These ratios define taxes paid to firms such that the costs of provision are exactly covered and no firm earns profits. The problem of how positive profits should be redistributed would not be addressed until Mas-Colell and Silvestre (1989). Their theory of cost-share equilibrium endogenously determines profit shares for a public firm, and shows that profits should be distributed according to the benefit an individual receives from a public good. This result built on Mas-Colell's (1980) previous work, in which he showed that when costs do not have a linear structure Lindahl's original result is still efficient and able to be attained in a decentralized manner. Diamantaras and Gilles (1996) extended this result by showing it continues to hold in the presence of an arbitrary number of private goods.  
...
There will be \(i = 1, ..., n \geq 2\) individuals in the economy, each represented by a convex consumption set \(C_i = \mathbb{R}^2_+\) and an initial endowment of a private good \(\omega_i > 0\). They consume bundles of a public good \(x\) and a private good \(y_i\), and in doing so gain utility according to utility functions of the following form.
...
This functional form simplifies later analysis, as \(v_i\) is the marginal rate of substitution between the public good and the private good for individual \(i\); \(\frac{\partial u_i}{\partial x} = v_i(x) > 0\) and \(\frac{\partial^2 u_i}{\partial x^2} = v_i'(x) < 0\) for \(x \in \mathbb{R}_+\).
...
\caption{An example of a classical Lindahl equilibrium for \(n=2\) consumers. The vertical grey dotted line is located at the Pareto optimal level of the public good. Consumer 1 is charged nothing, while consumer 2 is charged 5 units of the private good.}
...
This illustrates how a Lindahl equilibrium does not take into account the distribution of surplus among consumers when optimizing the provision of a public good. Even though the outcome is individually rational and Pareto optimal, some consumers may be left far better off than others. In order to address this flaw, we introduce the concept of a generalized Lindahl equilibrium. 

\section{Generalized Lindahl Equilibrium}

A classical Lindahl equilibrium must be individually rational, Pareto optimal, and raise tax revenue equal to the cost of providing of the public good. As stated in the previous section, the disregard of consumer surplus in such an equilibrium can lead to unfair outcomes. In this section, we explain how a generalization of the classical Lindahl equilibrium can allow a social planner to construct far more equitable outcomes. 
...
We can imagine a society's preferences for the distribution of surplus among its constituents being encoded into a marginal social welfare function. The maximization of the integral of this function, subject to the conditions necessary for a generalized Lindahl equilibrium stated in the previous section, is a theoretical representation of how such a society could provide a public good in an equitable and Pareto optimal way. 
...
\noindent Where \(\sigma = \frac{1}{1 - \alpha}\) is the elasticity of substitution between the surpluses of any two individuals. I will define \( \Omega_{21} = \left( \beta_2 / \beta_1 \right)^\sigma\). Substituting this and rearranging, we get

\[
p_2 - \Omega_{21} p_1 = v_2 - \Omega_{21}v_1
\]

\noindent If we perform a similar manipulation among all \(n\) Euler equations, we are left with a system of \(n-1\) equations
...
The final qualification left to prove is that the cost of provision is covered. This is true via the modulo construction of the personalized tax outcome function. No matter what messages are submitted, the sum of all tax schedules \(\tau_i\) equals marginal cost at every single margin, \(\sum_{i=1}^n \tau_i = c\). 
...
\end{proposition}
	...
	This would theoretically allow a social planner to create a mechanism that attains whatever generalized Lindahl outcome it desires. In order to prove proposition 2, the generalized Walker mechanism we currently have will have to be adjusted to elicit more information. Our outcome functions \ref{oc_x} and \ref{oc_t} do not elicit enough.
...
If this system implicitly defines a unique set of messages (\(p_1, ...,p_n\)), then that set of messages uniquely implements the generalized Lindahl outcome described by (\(\tau_1, ..., \tau_n\)) and (\(x, y_1, ..., y_n\)) and we have proved proposition 2. Unfortunately, this system does not have enough information to do so. The messages are functions, and so need to be defined across all margins of provision to be unique. The \(n-1\) personalized tax equations provide such definitions, but the outcome function that dictates the level of the public good is only defined on a single margin, \(x\). The only constraint it puts on the messages is that their sum must equal zero at \(x\). Consequently, this system defines an infinite number of Nash equilibria capable of achieving any single generalized Lindahl outcome. Proving proposition 2 is impossible with the mechanism as it stands. 
...
The point of complication arises in definition 3 with the message space \(M\). We need \(m \in M\) to be able to generate a tax schedule via some outcome function. It is hard to see how to do so without having \(M\) include some space of continuous functions. If, for example, we choose to work with a message space \(M = \mathbb{C}(\mathbb{R})^n\), we would need to create a distance metric that somehow quantifies the distance between a pair of continuous functions. Such a distance metric may make it very difficult to prove a mechanism is contractive. 

\section{Conclusion}

Our generalized Lindahl equilibrium is an extension of a classical Lindahl equilibrium. The classical result has desirable properties. It is Pareto optimal, individually rational, and covers the cost of providing a public good. But, it can lead to unjust outcomes because it does not take into account the distribution of consumer surplus. A generalized Lindahl equilibrium corrects this shortcoming, and retains said desirable features of a classical Lindahl equilibrium. 

We have shown how a generalized Walker mechanism allows a social planner to attain a generalized Lindahl equilibrium when they do not know the private valuations of consumers. We have proved that any Nash equilibrium of a game induced by this mechanism is a generalized Lindahl equilibrium. In future research, we would like to adjust our mechanism and prove that any generalized Lindahl equilibrium can be achieved by a unique Nash equilibrium.

Lindahl's classical result is a standard by which public goods theories are measured. Our generalization is a better standard for certain economies, because it is not satisfied by individuals merely being better off: \textit{how} they are better off matters, in relation to themselves and others. This new standard is applicable to any economy concerned with the distribution of surplus; and any economy not so concerned is potentially unjust.


\section*{Appendix}

The solution for closed forms of the personalized tax functions in section 2 will take place as follows: I will solve for \(p_1\) in terms of \(p_2, v_1\) and \(v_2\) using the first equation from the system, and then substitute \(p_1\) into \ref{iso_simp}; I will iterate this process two more times, substituting equations for \(p_2\) and \(p_3\) into \ref{iso_simp}; at that point a pattern will emerge that will allow me to extrapolate a closed-form solution for \(p_n\), and then for \(p_{n-1}\); finally, I will use the pattern observable in the solutions for \(p_n\) and \(p_{n-1}\) to deduce a closed form solution for \(p_j\) in general.
...
Let's quickly look at all three instances of the pattern.
...
To get a closed form solution for any \(p_j\), I'll first use 6 to solve for \(p_{n-1}\) using the equation from our system of equations that relates \(p_{n-1}\) to \(p_n\). Then with closed from solutions for both \(p_n\) and \(p_{n-1}\), a pattern emerges that I use to solve for the general closed form solution for \(p_i\).
...
Substitute in \ref{p_n}. Step by step simplification:
...
\]
 ...
 \begin{equation}
 p_{n-1} =  \left( \sum_{i=1}^n \Omega_{i, n-1} \right)^{-1} \left[ c - \sum_{i \neq n-1} v_i + v_{n-1} \sum_{i \neq n-1} \Omega_{i, n-1}  \right] \label{p_n-1}
 \end{equation}

Our closed form solution to \(p_{n-1}\). Finally let's compare \ref{p_n-1} to \ref{p_n}.
