...
\noindent Note that the subscript \(i + j\) is understood to be a modulo operation, \(n+j= j\), and \(c\) is once again the marginal cost of the public good.
...
\begin{align*}
&\max_{p_i(x)} ~ u_i \left( x, \omega_i - \int_0^x \left[ \frac{\beta}{n} + p_{i+2}(t) - p_{i+1}(t) \right] dt \right) \numberthis 
\\
&s.t.~ x = \argmax_{t \geq 0} \int_o^t \sum_{i=1}^n p_i(m) dm
\end{align*}
...
\noindent Remember that, even though the utility maximization problem was simplified into a choice over \(x\), the optimal level of the public good demanded \(x^*\) uniquely and implicity defines an optimal message \(p_j^*\) via the outcome function \ref{oc_x}: 

\[
x^* = \argmax_{t \geq 0} \int_o^t \left( p_j^*(m) + \sum_{i \neq j} p_i(m) \right) dm
\]


\noindent So, equation \ref{foc} states that in a Nash equilibrium a best responding individual submits a message \(p_i\) such that the level of the public good it implicitly demands is optimal given their personalized tax at the margin. If we assume that \(u_i\) is quasi-concave then \ref{foc} defines a global maximum, and we can state that a Nash equilibrium is individually rational for all players to accept. 
...
\noindent The modulo dependence of the personalized taxes creates a cancellation that leads to two important results. First, the sum of all marginal rates of substitution are equal to the marginal cost of provision: the outcome is Pareto optimal. Second, the sum of all personalized taxes equals marginal cost at every marginal provision of the public good, so the cost of providing \(x\) will always be covered. 
